...
export default function WorkbenchSyncStatus({
  ...
  if (state === "saving") return <span className="flex items-center gap-1 text-primary"><Loader2 className="h-3 w-3 animate-spin" />正在保存到云端</span>;
  if (state === "local-draft") return <span className="flex items-center gap-1 text-warning"><CloudOff className="h-3 w-3" />本机草稿，尚未保存到云端</span>;
  ...
  if (state === "error") return <span className="flex items-center gap-1 text-destructive"><CloudOff className="h-3 w-3" />保存失败，本机草稿已保留</span>;
  if (state === "synced") {
    return <span className="flex items-center gap-1 text-success"><Save className="h-3 w-3" />已保存{lastSavedAt ? ` ${lastSavedAt.toLocaleTimeString()}` : ""}</span>;
  }
  return null;
}
