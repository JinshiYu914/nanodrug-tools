...
8. **加样体系**：LNP 浓度 100 ng/µL、**投料 RNA 10 µg**、linker 0.5 mol%、N/P 6、阳离子 50%、
...
12c. **模块 4 体内**：粘贴四列（样本名 / 器官 / Total ROI / Avg ROI）后**一行三张图**：
    Total ROI 分组柱状图、Avg ROI 分组柱状图、肝/脾占比堆叠图（**纵轴上限固定为 1**，
    拟合纵轴会让 80/20 看起来像 100/0）。样本名里带空格不能被拆成两列
...
13. **导出**：批次总览导出 PDF（**用含中文的样品名**，确认 CJK 正常）、Excel、JSON 三种都能出；
    ...
    「体外结果」表底部要有 均值 / SD / n 三行，「体内结果」表底部要有肝脾占比。
